...
def _whole_unit_allocations(results: list[dict[str, Any]], desired: list[float], weights: list[float], *, budget: float, target_deployable: float, cap_fraction: float, reserve_floor: float, cap_is_hard: bool, risk_tolerance: str, category_cap_fractions: dict[str, float] | None = None) -> tuple[list[float], dict[str, Any]]:
    ...
    candidate_limit = max(0.0, budget * cap_fraction)
    ...
    for item in results:
        ...
        if not math.isfinite(price) or price <= 0:
            price = 0.0
        prices.append(price)
    order = sorted(range(n), key=lambda i: (float(weights[i]) if i < len(weights) else 0.0, float(results[i].get("score") or 0.0), float(results[i].get("confidence") or 0.0), -i), reverse=True)
    current = 0.0
    for _ in range(10000):
        ...
        for i in order:
            ...
            if next_candidate > candidate_limit + 0.005 or next_total > integer_ceiling + 0.005:
                continue
            category = _category_from_item(results[i])
        ...
        if chosen is None:
            break
        price = prices[chosen]
        category = _category_from_item(results[chosen])
        units[chosen] += 1
    ...
    return allocations, {"whole_unit_target": round(target, 2), "whole_unit_integer_ceiling": round(integer_ceiling, 2), "whole_unit_soft_candidate_cap": round(candidate_limit, 2), "whole_unit_cap_is_hard": True, "whole_unit_auto_lot_override_candidates": 0, "whole_unit_auto_lot_override_used": 0, "whole_unit_adjustment": round(sum(allocations) - target, 2)}
