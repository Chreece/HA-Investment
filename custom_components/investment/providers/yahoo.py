...
class YahooProvider(MarketProvider):
    ...
    async def async_discover(
        ...
        async def append_dynamic(cat: str) -> None:
            fallback = _DISCOVERY_FALLBACK.get(cat, ())
            reserve = min(3, len(fallback)) if cat == "etf" else 0
            dynamic_limit = max(0, max_rows - reserve)
            for screener_id in _DISCOVERY_SCREENERS.get(cat, ()):
                if sum(1 for row in rows if row.category == cat) >= dynamic_limit:
                    break
                try:
