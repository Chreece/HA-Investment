...
def test_backend_and_frontend_use_same_versioned_component_identity():
    ...
    assert 'PANEL_NAME = "investment-panel-r37"' in const
    assert "webcomponent_name=PANEL_NAME" in init
    assert 'customElements.get("investment-panel-r37")' in base
    assert 'customElements.define("investment-panel-r37",InvestmentPanel)' in base
    assert 'const Panel = customElements.get("investment-panel-r37")' in runtime
