...
def test_v14g5_runtime_bootstrap_is_synchronous_and_desktop_safe():
    ...
    assert 'import "./investment-panel.js?v=0.4.0-r40";' in panel
    assert 'const Panel = customElements.get("investment-panel-r40")' in panel
...
def test_base_panel_uses_fresh_versioned_custom_element_for_mobile_webview_upgrade():
    ...
    assert 'if(!customElements.get("investment-panel-r40")) customElements.define("investment-panel-r40",InvestmentPanel);' in panel
...
def test_v14g5_frontend_revision_and_component_identity_are_bumped():
    const = text("const.py")
    runtime = text("www/investment-panel-runtime.js")
    assert 'PANEL_ASSET_REVISION = "0.4.0-r40"' in const
    assert 'investment-panel.js?v=0.4.0-r40' in runtime
    assert 'PANEL_NAME = "investment-panel-r40"' in const
